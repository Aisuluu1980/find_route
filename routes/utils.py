...
def dfs_paths(graph, start, goal):
    ...
    # одного и того же города более одного раза не рассматривается start-откуда выезжаем, goal-куда приедем
...
def get_routes(request, form) -> dict:
    ...
    all_ways = list(dfs_paths(graph, from_city.id, to_city.id))   #список список, набор маршрутов
    if not len(all_ways):
    ...
    if cities:
        ...
        for route in all_ways:
            if all(city in route for city in _cities):   #проходим циклом по городам , если есть город в маршруте, то добавляется в список
    ...
    for route in right_ways:
        ...
        if total_time <= traveling_time:
            # если общее время в пути меньше заданного, то добавляем маршрут в общий список
            routes.append(tmp)
    if not routes:
        # если список пуст, то нет никаких маршрутов, которые удовлетворяли бы заданным условиям
